...
- Host CDP port is exactly `ports.Block.Start + 5`.
...
```go
if urls["browserCdp"] != "http://localhost:23005" {
	t.Fatalf("browser CDP URL = %#v", urls["browserCdp"])
}
portsData, ok := got["ports"].(map[string]any)
if !ok || portsData["browserCdp"] != float64(23005) {
...
```go
if !strings.Contains(got, "Browser CDP: http://localhost:23005") {
...
```go
if spec.Ports.Size < 6 || spec.Ports.Start < 1 || spec.Ports.End() > 65535 {
...
**Interfaces:**
- Consumes: public fields `browserCdp`, port offset `+5`, container port `9222`
...
required := map[string][]string{
	"README.md": {"Browser CDP", "browserCdp", "23005"},
	"docs/configuration.md": {"9222", "offset `+5`", "always running"},
	"docs/security.md": {"CDP", "cookies", "127.0.0.1"},
	"README.pt-BR.md": {"CDP do navegador", "browserCdp"},
	"docs/pt-BR/configuration.md": {"9222", "offset `+5`"},
...
- [x] Graphiti contains the durable mapping between Webtop Chromium, CDP 9222,
  host offset `+5`, status fields, and lifecycle supervision.
